...
plt.legend([curve,curve1,curve2],["HermiteS","HermiteS_v1","Hermite_v2"],loc="upper right")
...
def PlotBezierCurve_v1(Polygon):
    N = len(Polygon[0, :])-1
    t = np.linspace(0,1,200)
    Bern = Bernstein(N, t)
    Bezier = Polygon @ Bern
...
def PlotBezierCurve_v2(Polygon):
    N = len(Polygon[0, :])-1
    t = np.linspace(0,1,200)
    Bern = Bernstein(N, t)
    Bezier = Polygon @ Bern
...
def PlotHermiteSpline_v1(Poly, alpha=0.1):
    ...
    for i in range(n - 1):
        ...
        # Utilisation de De Casteljau pour interpoler les points de Bézier
        PlotBezierCurve_v1(bezier_points)
...
def PlotHermiteSpline_v2(Poly):
    ...
    for i in range(n - 1):
        ...
        # Utilisation de De Casteljau pour interpoler les points de Bézier
        PlotBezierCurve_v2(bezier_points)
...
class Index(object):
    ...
    def newPoly(self, event):
        ...
        PlotHermiteSpline_v2(Poly)
        PlotHermiteSpline_v1(Poly,other_slider.val)
...
other_slider = Slider(other_ax_slider, label='α', valmin=0.1, valmax=1, valinit=0.5)
